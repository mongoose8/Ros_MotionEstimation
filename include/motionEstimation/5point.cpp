...
bool Solve5PointEssential(std::vector<cv::Point2f>& pts1, std::vector<cv::Point2f>& pts2, std::vector<int>& indices_to_use, int num_pts, std::vector<cv::Mat> &ret_E, std::vector<cv::Mat> &ret_P, std::vector<int> &ret_inliers, std::vector<std::vector<int>>& ret_max_inlier_indices, std::vector<float>& ret_avgRepoErr)
{
    int n_pts = indices_to_use.size();
    assert(num_pts >= 5);

    ret_E.clear();
    ret_P.clear();
    ret_inliers.clear();
    ret_max_inlier_indices.clear();
    ret_avgRepoErr.clear();

    // num_pts is the number of all points
    if(num_pts < 5) {
        return false;
    }

    // F is a temp variable, not the F fundamental matrix
    cv::Mat F(n_pts, 9, CV_64F);

    for(int i=0; i < n_pts; i++) {

        F.at<double>(i,0) = pts1[indices_to_use[i]].x*pts2[indices_to_use[i]].x;
        F.at<double>(i,1) = pts2[indices_to_use[i]].x*pts1[indices_to_use[i]].y;
        F.at<double>(i,2) = pts2[indices_to_use[i]].x;
        F.at<double>(i,3) = pts1[indices_to_use[i]].x*pts2[indices_to_use[i]].y;
        F.at<double>(i,4) = pts1[indices_to_use[i]].y*pts2[indices_to_use[i]].y;
        F.at<double>(i,5) = pts2[indices_to_use[i]].y;
        F.at<double>(i,6) = pts1[indices_to_use[i]].x;
        F.at<double>(i,7) = pts1[indices_to_use[i]].y;
        F.at<double>(i,8) = 1.0;
    }
...
//    cv::Mat x2(3, 1, CV_64F);
    ...
    for(size_t i=0; i < solutions.size(); i++) {
        ...
        for(int j=0; j < 4; j++) {
            ...
            int inliers = 0; // number of points in front of the camera

            // additionally compute backprojection error!
            ...
            for(int k=0; k < num_pts; k++) {
                ...
                // store all inlier coordinates
                if(depth1 > 0 && depth2 > 0) {

                    points3d.push_back(cv::Point3f(float(pt3d.at<double>(0,0)/w), float(pt3d.at<double>(1,0)/w), float(pt3d.at<double>(2,0)/w)));
                    points2d.push_back(pts2[k]);

                    max_inlier_indices.push_back(k);
                    inliers++;
                }
            }

            if(inliers > 1) {
                // compute repo error at first
                cv::Mat Pconv;
                P[j].convertTo(Pconv, CV_32F);
                computeReprojectionError(Pconv, points2d, points3d, pReprojected, reprojectionErrors, avgReprojectionError);
                float err = (avgReprojectionError.x+avgReprojectionError.y)/2.0;
...
//                std::cout<<" "<<std::endl;
                ...
                if (err < min_repoErr){
                    ...
                    E.copyTo(best_E);
                    P[j].copyTo(best_P);
                    copy(max_inlier_indices.begin(), max_inlier_indices.end(), std::back_inserter(best_inlier_indices));
                    found = true;
                }
            }
        }
        ...
        if(found) {
            ret_avgRepoErr.push_back(min_repoErr);
            ret_E.push_back(best_E);
            ret_P.push_back(best_P);
            ret_inliers.push_back(best_inliers);
            ret_max_inlier_indices.push_back(best_inlier_indices);
        }
    }
