...
vector<errors> calcSequenceErrors (vector<Matrix> &poses_gt,vector<Matrix> &poses_result, int dataset) {
  ...
  // KITTI dataset (2): Step size = 10 (10 images per second)
  // KITTI dataset (1): Step size = 30 (30 images per second)
  if (dataset == 1){step_size = 30;}else{step_size = 10;} // every second
  if (dataset == 1){factor_speed = 1.0/30.0;}else{factor_speed = 1.0/10.0;}

  // pre-compute distances (from ground truth as reference)
  vector<float> dist = trajectoryDistances(poses_gt);
 
  float lengths[] = {100,200,300,400,500,600,700,800};
  //if (dataset == 1){for (int32_t i = 0; i < num_lengths; i++){lengths[i]   = float(5.0*(i+1))/100.0;}}
  if (dataset == 1){for (int32_t i = 0; i < num_lengths; i++){lengths[i]   = float(i+1)*2.0;}}
  ...
  for (int32_t first_frame=0; first_frame<poses_gt.size(); first_frame+=step_size) {
    ...
    for (int32_t i=0; i<num_lengths; i++) {
      ...
      float speed = len/(factor_speed*num_frames);
...
void saveErrorPlots(vector<errors> &seq_err,string plot_error_dir,char* prefix, int dataset) {
  ...
  for (int32_t i=0; i<num_lengths; i++) {
    ...
    for (vector<errors>::iterator it=seq_err.begin(); it!=seq_err.end(); it++) {
      if (fabs(it->len-lengths[i])<thresh) {
        std::cout<<fabs(it->len-lengths[i])<<std::endl;
        t_err += it->t_err;
        r_err += it->r_err;
        num++;
      }
...
bool eval(int dataset) {
  ...
  string gt_dir         = "/home/jinhwa/Downloads/MotionEstimation/gtposes/";
  string result_dir     = "/home/jinhwa/Downloads/MotionEstimation/results/";
  ...
  for (int32_t i=11; i<22; i++) {
    ...
    // file name
    char file_name_gt[256];
    sprintf(file_name_gt,"%02d.txt",i-11);


    // read ground truth and result poses
    vector<Matrix> poses_gt     = loadPoses(gt_dir + file_name_gt);
    vector<Matrix> poses_result = loadPoses(result_dir + file_name);

    // check for errors
    if (poses_gt.size()==0 || poses_result.size()!=poses_gt.size()) {
        std::cout<<"No poses available for dataset "<<file_name<<". Skipping to next potential dataset."<<std::endl;
        continue;
    }

    // compute sequence errors    
    vector<errors> seq_err = calcSequenceErrors(poses_gt,poses_result, dataset);
    ...
    if (i<=15) {
      ...
      saveErrorPlots(seq_err,plot_error_dir,prefix, dataset);
  ...
  if (total_err.size()>0) {
    char prefix[16];
    sprintf(prefix,"avg");
    saveErrorPlots(total_err,plot_error_dir,prefix, dataset);
    plotErrorPlots(plot_error_dir,prefix);
...
void evaluate_odometry(int dataset){
  ...
  // run evaluation
  bool success = eval(dataset);
  if (success == true){std::cout<<"Evaluation was sucessful!"<<std::endl;}
  else{std::cout<<"Evaluation was NOT sucessful!"<<std::endl;}
