                          ...
                          cv::Mat& T, cv::Mat& R)
{
    ...
    //cv::solvePnPRansac(pointCloud_1LR, imgPoints, K, distCoeffVec, rvec, T, false, 1000, 0.006 * maxVal, 0.25 * (float)(imgPoints.size()), inliers, CV_EPNP);

    cv::solvePnPRansac(pointCloud_1LR, imgPoints, K, distCoeffVec, rvec, T, false, 1000, 2.0, 0.75 * (float)(imgPoints.size()), inliers, CV_EPNP);
...
// ensure that the first two singular values are 1 and the third is zero
    cv::SVD svd(E, cv::SVD::FULL_UV); //MODIFY_A
    cv::Mat svd_u = svd.u;  // U
    cv::Mat svd_w = svd.w; // D
    cv::Mat svd_vt = svd.vt; // V transpose

//    std::cout<<"U: "<<svd_u<<std::endl;
//    std::cout<<"W: "<<svd_w<<std::endl;
//    std::cout<<"Vt: "<<svd_vt<<std::endl;


    if ( fabsf(fabsf(svd_w.at<float>(0)) -fabsf(svd_w.at<float>(1) - 1.0) ) > 0.01){
        svd_w.at<float>(0) = (svd_w.at<float>(0) + svd_w.at<float>(1))/2.0;
    }

    cv::Mat W = cv::Mat::eye(3, 3, CV_32F);
    W.at<float>(0,0) = svd_w.at<float>(0);
    W.at<float>(1,1) = svd_w.at<float>(0);
    W.at<float>(2,2) = 0.0;
